        ...
        _comm->set_recv_callback([&](std::vector<uint8_t> buf) {
            ...
            if (buf[0] == CMD_READ_KEYBOARD_INPUT && buf[1] == 0x3F) {
                ...
                if(keyboard_callback_) {
                    keyboard_callback_((uint8_t)key);
                }
            }
            else if (buf[0] == CMD_SEARCH_CARD_START && buf[1] == CMD_SEARCH_CARD_RESULT) {
                if(search_card_callback_) {
                    ...
                    if(ln >= 4) {
                        ...
                        search_card_callback_(code, type, key);
                    }
                }
            }
            else {
...
int future_devices::send_request_cmd(uint8_t cmd, std::vector<uint8_t> params){
...
int future_devices::recv_from_list(int8_t cmd, std::vector<uint8_t> &buf){
    int ret = SDK_TIMEOUT;
    ...
    while (timeout > 0)
    {
        if (recv_list.size() > 0)
        {
            if (recv_list[0][0] == cmd && recv_list[0][1] == 0x4F)
            {
                std::vector<uint8_t> tmp = recv_list[0];
...
std::vector<uint8_t> future_devices::lvar_to_vector(std::vector<uint8_t> buf) {
    int ln = buf[0];
    return std::vector<uint8_t>(buf.begin() + 1, buf.begin() + 1 + ln);
}

std::vector<uint8_t> future_devices::llvar_to_vector(std::vector<uint8_t> buf) {
    int ln = buf[0] * 256 + buf[1];
    return std::vector<uint8_t>(buf.begin() + 2, buf.begin() + 2 + ln);
}

int future_devices::set_communication_mode(uint8_t mode) {
...
int future_devices::get_network_mode(uint8_t &mode) {
...
int future_devices::get_system_version(uint8_t type, std::vector<uint8_t>& version) {
...
int future_devices::get_hardware_serial_number(std::vector<uint8_t>& serial_number) {
...
int future_devices::get_device_model(uint8_t &model) {
...
int future_devices::get_customer_serial_number(std::vector<uint8_t>& custom_serial_number) {
...
int future_devices::set_clock(std::vector<uint8_t> time) {
...
int future_devices::get_clock(std::vector<uint8_t>& time) {
...
int future_devices::beep_frequency(uint32_t frequency, uint32_t duration) {
...
int future_devices::led_on(uint32_t color) {
    std::vector<uint8_t> tmp;   
...
int future_devices::led_off(uint32_t color) {
    std::vector<uint8_t> tmp;   
    tmp.push_back((uint8_t)((color>>24)&0xFF));
    tmp.push_back((uint8_t)((color>>16)&0xFF));
    tmp.push_back((uint8_t)((color>>8)&0xFF));
...
int future_devices::led_flash(uint32_t color, uint32_t duration) {
    std::vector<uint8_t> tmp;   
    tmp.push_back((uint8_t)((color>>24)&0xFF));
    tmp.push_back((uint8_t)((color>>16)&0xFF));
    tmp.push_back((uint8_t)((color>>8)&0xFF));
...
int future_devices::system_reset() {
...
int future_devices::system_shutdown() {
...
int future_devices::set_terminal_serial_number(std::vector<uint8_t> serial_number, std::vector<uint8_t> custom_serial_number) {
    std::vector<uint8_t> tmp;
...
int future_devices::get_chip_serial_number(std::vector<uint8_t>& serial_number) {     
...
int future_devices::set_enable_sleep_mode(uint32_t wakeup_type) {
    std::vector<uint8_t> tmp;
...
int future_devices::get_enter_boot_state(uint8_t type, uint32_t& state) {
...
int future_devices::keyboard_open(std::function<void(uint8_t)> callback) {
...
int future_devices::keyboard_close() {
    keyboard_callback_ = NULL;
...
int future_devices::keyboard_clear_cache() {
...
int future_devices::keyboard_set_sound(uint8_t sound) {
...
int future_devices::keyboard_set_backlight(uint8_t backlight) {
...
int future_devices::tp_open(uint8_t report) {
    send_request_cmd(CMD_OPEN_TP, {report});
...
int future_devices::tp_check_support() {
...
int future_devices::tp_get_touch_coordinate(uint32_t &x, uint32_t &y) {
...
int future_devices::tp_set_parameter(uint32_t start_x, uint32_t start_y, uint32_t end_x, uint32_t end_y, uint32_t interval) {
    std::vector<uint8_t> tmp;
...
int future_devices::magnetic_stripe_card_open() {
...
int future_devices::magnetic_stripe_card_close() {
...
int future_devices::magnetic_stripe_card_check() {
...
int future_devices::magnetic_stripe_card_read_data(std::vector<uint8_t>& track1, std::vector<uint8_t>& track2, 
...
int future_devices::magnetic_stripe_card_clear_data() {
...
int future_devices::magnetic_stripe_card_format_data(std::vector<uint8_t> track1, std::vector<uint8_t> track2, 
...
int future_devices::ic_card_open(uint8_t card_type, uint8_t card_seat) {
    std::vector<uint8_t> tmp;
...
int future_devices::ic_card_close(uint8_t card_type, uint8_t card_seat) {
    std::vector<uint8_t> tmp;
    tmp.push_back(card_type);
    tmp.push_back(card_seat);
...
int future_devices::ic_card_check_status(uint8_t card_type, uint8_t card_seat) {
    std::vector<uint8_t> tmp;
    tmp.push_back(card_type);
    tmp.push_back(card_seat);
    ...
    std::vector<uint8_t> buf;
    int ret = recv_from_list(CMD_CHECK_IC_STATUS, buf);
    return ret;
}

int future_devices::ic_card_reset(uint8_t card_type, uint8_t card_seat, 
                                  std::vector<uint8_t>& reset_data) {
    std::vector<uint8_t> tmp;
    tmp.push_back(card_type);
    tmp.push_back(card_seat);
    for(size_t i = 0; i < reset_data.size(); i++) {
        tmp.push_back(reset_data[i]);
    }
...
int future_devices::ic_card_module_power_off(uint8_t card_type, uint8_t card_seat) {
    std::vector<uint8_t> tmp;
    tmp.push_back(card_type);
    tmp.push_back(card_seat);
...
int future_devices::ic_card_send_apdu_command(uint8_t card_type, uint8_t card_seat, 
...
int future_devices::icc_open_module() {
...
int future_devices::icc_close_module() {
...
int future_devices::icc_search_card_activation(uint8_t card_type, uint8_t &card_type_return, 
...
int future_devices::icc_send_apdu_command(std::vector<uint8_t> apdu_command, std::vector<uint8_t> card_return_data) {
    std::vector<uint8_t> tmp;
    tmp.push_back((uint8_t)((apdu_command.size()>>8)&0xFF));
    tmp.push_back((uint8_t)(apdu_command.size()&0xFF));
    for(size_t i = 0; i < apdu_command.size(); i++) {
...
int future_devices::mifare_card_authentication(uint8_t block_number, uint8_t key_type, 
...
int future_devices::mifare_card_operation(uint8_t operation_instruction, uint8_t block_number, uint8_t target_block_number, 
...
int future_devices::search_card_start(uint8_t search_mode, uint32_t timeout_ms, 
        std::function<void(int32_t code, uint8_t type, uint8_t key)> callback) {
    search_card_callback_ = callback;
...
int future_devices::search_card_stop() {
    search_card_callback_ = nullptr;
...
int future_devices::scan_read_data(uint32_t timeout_ms, std::vector<uint8_t>& data) {
    std::vector<uint8_t> tmp;
    tmp.push_back((uint8_t)((timeout_ms>>24)&0xFF));
    tmp.push_back((uint8_t)((timeout_ms>>16)&0xFF));
    tmp.push_back((uint8_t)((timeout_ms>>8)&0xFF));
...
int future_devices::printer_check_support() {   
...
int future_devices::printer_open() {
...
int future_devices::printer_close() {
...
int future_devices::printer_query_status() {
...
int future_devices::printer_set_gray(uint8_t gray) {
...
int future_devices::printer_paper_feed(uint32_t pixel_point) {  
    std::vector<uint8_t> tmp;
...
int future_devices::printer_print_bitmap_data(uint8_t packet_number, std::vector<uint8_t> bitmap_data, 
...
int future_devices::keypad_open() {
...
int future_devices::keypad_close() {
...
int future_devices::keypad_get_random_number(uint32_t length, std::vector<uint8_t>& random_number) {
    std::vector<uint8_t> tmp;
...
int future_devices::keypad_update_master_key(uint32_t master_key_index, std::vector<uint8_t> master_key_data, 
...
int future_devices::keypad_update_work_key(uint32_t master_key_index, std::vector<uint8_t> pin_key_data, 
...
int future_devices::keypad_encrypt_data(uint32_t master_key_index, uint8_t work_key_type, std::vector<uint8_t> iv, 
...
int future_devices::keypad_encrypt_magnetic_stripe_data(uint32_t master_key_index, uint8_t magnetic_stripe_encryption_mode, 
...
int future_devices::keypad_calculate_mac(uint32_t master_key_index, std::vector<uint8_t> data, uint8_t mac_algorithm_mode, 
...
int future_devices::keypad_input_online_pin(uint32_t master_key_index, std::vector<uint8_t> pin_length, uint8_t row_number, uint8_t column_number, 
...
int future_devices::keypad_generate_rsa_key_pair_output_public_key(uint32_t expected_key_length, std::vector<uint8_t> public_exponent, 
...
int future_devices::keypad_encrypt_rsa_private_key(std::vector<uint8_t> input_data, std::vector<uint8_t>& encrypted_data) {
    std::vector<uint8_t> tmp;
...
int future_devices::keypad_encrypt_hardware_serial_number(std::vector<uint8_t> data, uint8_t mode, std::vector<uint8_t>& encrypted_data) {
    std::vector<uint8_t> tmp;
    tmp.push_back((uint8_t)((data.size()>>8)&0xFF));
    tmp.push_back((uint8_t)(data.size()&0xFF));
    for(size_t i = 0; i < data.size(); i++) {
...
int future_devices::keypad_check_trigger_status(uint8_t &reset_check_switch, uint8_t &self_destruction_flag) {
...
int future_devices::keypad_set_trigger_status(uint32_t debounce_time, uint8_t reset_check_switch) {
    std::vector<uint8_t> tmp;
...
int future_devices::keypad_release_trigger(uint8_t mode) {
    std::vector<uint8_t> tmp;
    tmp.push_back(mode);
...
int future_devices::keypad_check_key(uint8_t key_type, uint32_t key_index, std::vector<uint8_t>& kcv) {
    std::vector<uint8_t> tmp;
    tmp.push_back(key_type);
...
int future_devices::keypad_sm3_hash_algorithm(std::vector<uint8_t> data, std::vector<uint8_t>& hash) {
    std::vector<uint8_t> tmp;
    tmp.push_back((uint8_t)((data.size()>>8)&0xFF));
    tmp.push_back((uint8_t)(data.size()&0xFF));
    for(size_t i = 0; i < data.size(); i++) {
...
int future_devices::keypad_des_encrypt_decrypt_algorithm(uint8_t algorithm_mode, uint8_t encrypt_decrypt_mode, 
...
int future_devices::keypad_aes_encrypt_decrypt_algorithm(uint8_t algorithm_mode, uint8_t encrypt_decrypt_mode, 
        std::vector<uint8_t> iv, std::vector<uint8_t> data, std::vector<uint8_t> key, std::vector<uint8_t>& result  ) {
    std::vector<uint8_t> tmp;
    tmp.push_back(algorithm_mode);
    tmp.push_back(encrypt_decrypt_mode);
...
int future_devices::keypad_sm4_encrypt_decrypt_algorithm(uint8_t algorithm_mode, uint8_t encrypt_decrypt_mode, 
...
int future_devices::keypad_sm2_encrypt_decrypt_algorithm(uint8_t encrypt_decrypt_mode, std::vector<uint8_t> data, 
...
int future_devices::keypad_sm2_signature_algorithm(std::vector<uint8_t> public_key, std::vector<uint8_t> private_key, 
...
int future_devices::keypad_sm2_verify_algorithm(std::vector<uint8_t> signature, std::vector<uint8_t> public_key, 
        std::vector<uint8_t> uid, std::vector<uint8_t> data, std::vector<uint8_t>& result) {
    std::vector<uint8_t> tmp;
    for(size_t i = 0; i < 64; i++) {
...
int future_devices::file_download_start_download(uint8_t file_type, uint8_t compress_format, uint8_t parameter_update, uint8_t reserved, 
...
int future_devices::file_download(uint8_t packet_number, uint32_t file_data_offset, std::vector<uint8_t> file_data) {
    std::vector<uint8_t> tmp;
    tmp.push_back(packet_number);
...
int future_devices::internal_authentication(std::vector<uint8_t> random_number_r1, std::vector<uint8_t>& encrypted_random_number_r1, 
...
int future_devices::external_authentication_hardware_serial_number_download(std::vector<uint8_t> encrypted_random_number_r2, uint8_t self_destruction_reset_flag, 
...
int future_devices::external_authentication_unlock(std::vector<uint8_t> encrypted_r2_key2, uint8_t self_destruction_reset_flag) {
    std::vector<uint8_t> tmp;   
    for(size_t i = 0; i < 8; i++) {
...
int future_devices::external_authentication_encrypted_chip_id(std::vector<uint8_t> encrypted_r2_key2) {
    std::vector<uint8_t> tmp;   
    for(size_t i = 0; i < 8; i++) {
        tmp.push_back(encrypted_r2_key2[i]);
    }
...
int future_devices::external_authentication_reset_boot(std::vector<uint8_t> encrypted_r2_key2) {
    std::vector<uint8_t> tmp;   
    for(size_t i = 0; i < 8; i++) {
        tmp.push_back(encrypted_r2_key2[i]);
    }
