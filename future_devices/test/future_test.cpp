...
void test_beep()
{
    ...
    ret = _devices.beep_frequency(0, 0);
    if (ret != 0) {
        printf("ERROR: beep_frequency ret: %d\n", ret);
        return;
    }
...
{
    future::future_devices& _devices = future::future_devices::getInstance();
    int ret = _devices.keyboard_open([](uint8_t key_code){
        printf("keyboard_open key_code: %d\n", key_code);
    });
    if (ret != 0) {
...
void test_tp()
{
    ...
    ret = _devices.tp_set_parameter(0, 0, 320, 240, 10);
    if (ret != 0) {
    ...
    if (ret != 0) {
        printf("ERROR: tp_get_touch_coordinate ret: %d\n", ret);
        return;
    }
...
void test_ic()
{
    ...
    if (ret != 0) {
        printf("ERROR: ic_card_check_status ret: %d\n", ret);
        return;
    }
    ...
    printf("card_return_data: %s\n", card_return_data.data());

    ret = _devices.ic_card_module_power_off(0, 0);
    if (ret != 0) {
...
void test_search_card()
{
    ...
    int ret = _devices.search_card_start(SDK_SWIPE_CARD_HAND | SDK_SWIPE_CARD_ICC | SDK_SWIPE_CARD_RF | SDK_SWIPE_CARD_MAG, 
        10000, [](int32_t code, uint8_t type, uint8_t key){
        printf("search_card_start ret: %d, type: %d, key: %d\n", code, type, key);
    });
    if (ret != 0) {
