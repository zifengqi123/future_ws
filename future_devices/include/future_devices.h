#ifndef  FUTURE_DEVICES_H_
#define  FUTURE_DEVICES_H_
...
class future_devices {

private:
    future_devices();
    ~future_devices();
    future_devices(const future_devices&) = delete;
    future_devices& operator=(const future_devices&) = delete;

    future_comm* _comm;

    int recv_timeout = 1500;  //ms

    std::vector<std::vector<uint8_t>> recv_list;
    std::function<void(uint8_t)> keyboard_callback_;
    std::function<void(int32_t code, uint8_t type, uint8_t key)> search_card_callback_;

public:
    static future_devices& getInstance() {
        static future_devices instance;
    ...
    /**
     * @brief 打开键盘 (命令字: 0x30)
     * @param callback 回调函数 参数 key
     * @return 成功返回SDK_OK，失败返回错误码
     */
    int keyboard_open(std::function<void(uint8_t)> callback);
     ...
     * @param sound 按键音开关 @see SDK_KEYBOARD_SOUND_OFF, SDK_KEYBOARD_SOUND_ON
     * @return 成功返回SDK_OK，失败返回错误码
     */
     ...
     * @param backlight 背光值 @see SDK_KEYBOARD_BACKLIGHT_OFF, SDK_KEYBOARD_BACKLIGHT_ON
     * @return 成功返回SDK_OK，失败返回错误码
     */
     ...
     * @return 成功返回SDK_OK，失败返回错误码
     */
    int ic_card_module_power_off(uint8_t card_type, uint8_t card_seat);
